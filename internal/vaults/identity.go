...
// tags on a knowledge base) fails without touching the file. A new name moves the
// vault's folder to match; the returned path is where the vault is afterwards.
...
// renameFolder moves e's folder so its name is the new one, cleaned the way a
// repository's name is. It returns the folder the vault sits in afterwards, which is the
// one it already had when the name needs no move. A project at REPO/atlas keeps its
// folder: that name is part of the layout, not of the vault.
func renameFolder(h home.Home, cfg *home.Config, e registry.Entry, name string) (string, error) {
	if vault.HostRepo(e.Path) != "" {
		return e.Path, nil
	}
