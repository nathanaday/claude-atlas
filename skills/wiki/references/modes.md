...
- immutable captured sources under `.raw/captured/`;
- pages under `wiki/`;
- `wiki/index.md`, `wiki/log.md`, and `wiki/hot.md` as the lifecycle pages;
- the source ledger under `wiki/meta/ledgers/`;
- one plan for a requested scaffold;
- no network or plugin setup without separate approval.
...
4. Apply approved additions once through `plan` and `apply`.
5. Run `wiki-lint` and report unresolved findings.
